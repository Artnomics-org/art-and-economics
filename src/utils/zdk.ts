...
import { Ask, Bid, BidShares, DecimalValue, MediaData } from '../types/ContractTypes'
import { Decimal } from './decimal'
import { BigNumber, BigNumberish, BytesLike, ContractTransaction, ethers, Wallet } from 'ethers'
 ...
 * @param prevOwner
 */
export function constructBidShares(creator: number, owner: number, prevOwner: number): BidShares {
 ...
 * @param prevOwner
 */
export function validateBidShares(creator: DecimalValue, owner: DecimalValue, prevOwner: DecimalValue): void {
  ...
  if (sum.toString() !== decimal100.value.toString()) {
    invariant(false, `The BidShares sum to ${sum.toString()}, but they must sum to ${decimal100.value.toString()}`)
  }
}
 ...
 * @param amount
 */
export async function wrapETH(wallet: Wallet, wethAddress: string, amount: BigNumber): Promise<ContractTransaction> {
 ...
 * @param amount
 */
export async function unwrapWETH(wallet: Wallet, wethAddress: string, amount: BigNumber): Promise<ContractTransaction> {
